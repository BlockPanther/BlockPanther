...
unsigned int nTargetSpacing = 35; // 35 seconds
unsigned int nStakeMinAge = 1 * 24 * 60 * 60; // 1 day
unsigned int nStakeMaxAge = 31 * 24 * 60 * 60; // 31 days
unsigned int nModifierInterval = 35 * 60; // time to elapse before new modifier is computed

int nCoinbaseMaturity = 190; // 200 blocks for coins to mature
...
const string strMessageMagic = "BlockPanther Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    {
        nSubsidy = 8800000 * COIN; // For core developers
    }
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    else if(pindexBest->nHeight < 560000)
    {
        nSubsidy = nCoinAge * nRewardCoinYear * 100 / 365 / COIN;
    }
    else if(pindexBest->nHeight < 570000)
    {
        nSubsidy = nCoinAge * nRewardCoinYear * 75 / 365 / COIN;
    }
    else if(pindexBest->nHeight < 580000)
    {
        nSubsidy = nCoinAge * nRewardCoinYear * 50 / 365 / COIN;
    }
    else if(pindexBest->nHeight < 590000)
    {
        nSubsidy = nCoinAge * nRewardCoinYear * 25 / 365 / COIN;
    }
...
static const int64_t nTargetTimespan = 35 * 60;  // 35 mins
//
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BlockPanther", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (fTestNet)
    {
        ...
        nStakeMinAge = 12 * 60 * 60; // test net min age is 12 hours
        nCoinbaseMaturity = 90; // test maturity is 100 blocks
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "https://www.bbc.co.uk/news/uk-politics-47278902";  // Seven MPs leave Labour Party in protest at Jeremy Corbyn's leadership
        CTransaction txNew;
        txNew.nTime = 1550493000;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 4868357 << CBigNum(88) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("aa785c0a07b6c6b8b0f22d44e1175e43a1cef7870bd14bb13b9efd4c689c56dd"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x44, 0x77, 0xb1, 0xe4 };
